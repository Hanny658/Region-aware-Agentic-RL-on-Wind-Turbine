...
!-------------------------------------------------------------------------------------------------------------------------------  
    ...
    !       VS_State = VS_State_PI                (6), Tip-Speed-Ratio tracking PI controller (ignore state machine)
        ...
        ! PATCHED (region-aware RL project): apply the ZMQ torque residual AFTER the PI state
        ! reset above, so the native controller state never sees the offset (same principle as
        ! the pitch offset); saturate against the hard torque limits as a Fortran-side backstop.
        avrSWAP(47) = MAX(0.0_DbKi, saturate(LocalVar%VS_LastGenTrq + LocalVar%ZMQ_TorqueOffset, &
                                             CntrPar%VS_MinTq, CntrPar%VS_MaxTq))  ! Demanded generator torque, prevent negatives.
