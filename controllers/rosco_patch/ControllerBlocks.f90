...
!-------------------------------------------------------------------------------------------------------------------------------
    ...
    !       WE_Mode = 1, Use Inversion and Inveriance filter as defined by Ortege et. al. 
        ...
        IF (CntrPar%WE_Mode == 1 .AND. LocalVar%WE_Op > 0) THEN      
            ...
            ! PATCHED (region-aware RL): use the applied torque (native + ZMQ residual), see Filters.f90
            LocalVar%WE_VwIdot = CntrPar%WE_Gamma/CntrPar%WE_Jtot*((LocalVar%VS_LastGenTrq + LocalVar%ZMQ_TorqueOffset)*CntrPar%WE_GearboxRatio - Tau_r)
