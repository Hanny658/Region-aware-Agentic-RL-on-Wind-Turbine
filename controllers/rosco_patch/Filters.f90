...
!-------------------------------------------------------------------------------------------------------------------------------
    ...
    ! Prefilter shared measured wind turbine signals
        ...
        ! PATCHED (region-aware RL): the wind-speed estimator must see the *applied* torque
        ! (native + ZMQ residual) or its aero-torque balance integrates a bias (any nonzero
        ! residual drifts the wind estimate below rated). The VS PI state stays native.
        LocalVar%VS_LastGenTrqF = SecLPFilter(LocalVar%VS_LastGenTrq + LocalVar%ZMQ_TorqueOffset, LocalVar%DT, CntrPar%F_LPFCornerFreq, 0.7_DbKi, LocalVar%FP, LocalVar%iStatus, LocalVar%restart, objInst%instSecLPF)
